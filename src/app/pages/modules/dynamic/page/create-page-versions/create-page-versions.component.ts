...
export class CreatePageVersionsComponent {
  ...
  private initForm() {
    this.pageVersionForm = this.fb.group({
      ...
      selectedElement: ['', Validators.required],
    });
  ...
  fetchPagesCreated() {
    this.httpService
      .callApiServices(
        this.constant.serviceName_GETPAGESCREATED,
  ...
  addElement() {
    ...
    if (
      selectedValue &&
      !this.selectedElements.find((each) => each.DESCRIPTION === selectedValue)
        ?.DESCRIPTION
    ) {
      this.selectedElements = [
        ...this.selectedElements,
        this.elements.find((elem) => elem.DESCRIPTION === selectedValue),
      ];
  ...
  getDropDown() {
    ...
    this.httpService
      .callApiServices(GETDROPDOWN, inputData)
      .subscribe((data: any) => {
  ...
  searchPage() {
    ...
    this.httpService
      .callApiServices(GETDYNAMICPAGE, inputData)
  ...
  createPage() {
    ...
    if (!this.isPageCreated) {
      this.httpService
        .callApiServices(CREATEPAGE, inputData)
        .subscribe((data: any) => {
          const response = data.responseParameter;
  ...
  createPageElements() {
    const formattedArray = this.selectedElements
      .map((element, index) => {
    ...
    this.httpService
      .callApiServices(CREATEPAGEELEMENTS, inputData)
      .subscribe((data: any) => {
        const responseData = data.responseParameter;
        if (responseData.opstatus === '00') {
          this.dataService.information = `Page Version Created Successfully`;
          this.commonMethod.openPopup('div.popup-bottom.success-popup');
          setTimeout(() => {
            this.dataService.goToPage('getsavedData');
          }, 2000);
        } else {
          this.dataService.information = 'Unable to Create Page Version';
